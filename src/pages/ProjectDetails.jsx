  ...
  return (
    <section className="container-page py-10 md:py-16">
      <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-xl dark:border-slate-800 dark:bg-slate-950/70">
        ...
        <div className="grid gap-8 p-5 lg:grid-cols-[1.35fr_0.65fr] lg:p-8">
          <div>
            {project.image && (
              <img
                src={project.image}
                alt={project.title}
                className="mb-6 w-full rounded-xl border border-slate-200 object-cover dark:border-slate-800"
              />
            )}
            ...
            <div className="mt-8 rounded-xl border border-slate-200 p-5 dark:border-slate-800">
              ...
              <DetailSection title="Getting Started">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-600 dark:text-slate-300">
                  {details.prerequisites.map((item) => <li key={item}>{item}</li>)}
                </ul>
              </DetailSection>
              ...
              <DetailSection title="Why This Structure Works">
                ...
                <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-600 dark:text-slate-300">
                  {details.structurePoints.map((item) => <li key={item}>{item}</li>)}
                </ul>
              </DetailSection>
